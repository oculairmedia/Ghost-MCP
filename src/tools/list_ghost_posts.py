...
def list_ghost_posts(page: int = 1, limit: int = 15, status: str = None, include: str = "tags,authors") -> str:
    """
    ...
    Args:
        ...
        status (str, optional): Filter by post status (draft, published, scheduled)
...
if __name__ == "__main__":
    print("Testing list_ghost_posts tool...")
    
    # Example post listing
    print("\nListing posts...")
    try:
        posts = list_ghost_posts(page=1, limit=5, status="draft")
        response = json.loads(posts)
        if "error" in response:
            print(f"Error listing posts: {response['error']}")
        else:
